...
function buildNotification(c: Complaint, serial: number) {
  switch (c.status) {
    case "Assigned":
      return {
        text: `You have been assigned to Complaint #${serial}.`,
        icon: "assignment_ind",
    ...
    case "In Progress":
      return {
        text: `You have started working on #${serial}.`,
        icon: "play_circle",
        color: "text-sky-500",
        time: timeAgo(c.date),
      };
    ...
    default:
      return {
        text: `New update on ticket #${serial}.`,
        icon: "info",
        color: "text-primary",
        time: timeAgo(c.date),
      };
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Link
              href="/technician/view-complaints"
                      ...
                      {complaints.slice(0, 3).map((item, index) => (
                        ...
                        >
                          ...
                          <td className="py-4 px-6 text-right">
                            <Link
                              href={`/technician/view-complaints/${item.id}`}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 md:p-6 shadow-sm">
            {latestComplaint ? (
              <div className="space-y-6">
                ...
                <Link href={`/technician/view-complaints/${latestComplaint.id}`} className="block w-full py-3 bg-primary hover:bg-opacity-95 text-white rounded-xl font-bold text-center shadow-lg shadow-primary/20 transition-all text-sm cursor-pointer">
                  Go to task
                </Link>
              </div>
            ) : (
