...
export default function AddComplaint() {
  ...
  return (
    <div className="space-y-8 pb-12 max-w-4xl mx-auto">
      ...
      {/* Form card forced to light theme regardless of global dark mode */}
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-6 md:p-10 shadow-sm relative overflow-hidden">

        {/* Success Modal Overlay */}
        {success && (
          <div className="absolute inset-0 z-40 bg-white/95 flex flex-col items-center justify-center space-y-4 animate-fade-in text-center p-6">
            ...
            <h3 className="font-display text-2xl font-black text-slate-800">Ticket Filed Successfully!</h3>
        ...
        {submitting && (
          <div className="absolute inset-0 z-40 bg-white/80 flex flex-col items-center justify-center space-y-4 animate-fade-in">
            <div className="w-12 h-12 border-4 border-slate-200 border-t-primary rounded-full animate-spin" />
            <p className="text-sm font-bold text-slate-600">Publishing complaint ticket...</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label htmlFor="category" className="block font-bold text-xs uppercase tracking-widest text-on-surface-variant">
                Complaint Category
              </label>
              <select
                id="category"
                ...
                className="w-full rounded-xl px-4 py-3.5 font-body-md premium-input text-slate-800 cursor-pointer"
              >
            ...
            {/* Priority selection */}
            <div className="space-y-2">
              <label htmlFor="priority" className="block font-bold text-xs uppercase tracking-widest text-on-surface-variant">
                Priority Urgency
              </label>
              <select
                id="priority"
          ...
          {/* Location field */}
          <div className="space-y-2">
            <label htmlFor="location" className="block font-bold text-xs uppercase tracking-widest text-on-surface-variant">
              Campus Location
            </label>
            <input
              id="location"
              ...
              className="w-full rounded-xl px-4 py-3.5 font-body-md premium-input text-slate-800"
            />
          </div>

          {/* Description field */}
          <div className="space-y-2">
            <label htmlFor="description" className="block font-bold text-xs uppercase tracking-widest text-on-surface-variant">
              Issue Description
            </label>
            <textarea
              id="description"
              ...
              className="w-full rounded-xl p-4 font-body-md premium-input text-slate-800 resize-none"
            />
          </div>

          {/* Drag & drop Image upload mock */}
          <div className="space-y-2">
            <span className="block font-bold text-xs uppercase tracking-widest text-on-surface-variant">
            ...
            {!imagePreview ? (
              ...
              >
                ...
                <div>
                  <p className="font-bold text-sm text-slate-700">Drag & drop files or click to upload</p>
                  <p className="text-xs text-slate-400 mt-1">Supported: JPG, PNG, JPEG. Max file size: 5MB.</p>
                </div>
              </div>
            ) : (
              <div className="border border-slate-200 rounded-3xl p-4 flex items-center justify-between gap-4 bg-slate-50/50">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl overflow-hidden relative shrink-0 border border-slate-200">
          ...
          <div className="flex gap-4 pt-4 border-t border-slate-100">
            ...
            <button
              type="button"
              onClick={handleReset}
              className="px-6 py-4 border border-slate-200 text-slate-500 font-bold rounded-xl hover:bg-slate-50 transition-colors text-sm cursor-pointer"
            >
              Reset
            </button>
          </div>
