...
function actionFor(request: RecentRequest) {
  ...
  if (request.status === "Assigned" || request.status === "In Progress") {
    return (
      <Link
        href={`/admin/view-complaints/${request.id}`}
        className="text-[#c0c1ff] font-bold text-xs uppercase tracking-widest inline-flex items-center gap-1 hover:underline"
      >
    ...
    <div className="lg:col-span-3 bg-[#171f33] rounded-3xl border border-[#464554]/10 overflow-hidden vibrant-shadow">
      ...
      {requests.length === 0 ? (
        <div className="p-16 text-center">
          <Icon name="inbox" className="text-4xl text-[#908fa0] mb-3 block mx-auto" />
          <h4 className="font-display text-lg font-bold text-[#dae2fd]">No Requests Yet</h4>
          <p className="text-xs text-[#908fa0] mt-1">New maintenance tickets will show up here.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#464554]/10 font-medium text-sm">
              {requests.map((request) => (
                <tr key={request.id} className="hover:bg-[#8083ff]/5 transition-all group">
                  <td className="px-8 py-5 font-mono text-[#c0c1ff] font-bold">#{request.id}</td>
                  <td className="px-8 py-5">
                    <div className="font-bold text-[#dae2fd] group-hover:text-[#c0c1ff] transition-colors truncate max-w-xs">{request.title}</div>
                  ...
                  <td className="px-8 py-5">
                    {request.technicianName ? (
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-[#8083ff]/20 text-[#c0c1ff] flex items-center justify-center text-[10px] font-bold border border-[#8083ff]/20">
                          {initials(request.technicianName)}
                        </div>
                        <span className="text-xs font-semibold text-[#dae2fd]">{request.technicianName}</span>
                      </div>
                    ) : (
