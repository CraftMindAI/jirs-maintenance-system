          ...
          <div className="space-y-8">
            {CONTACT_DETAILS.map((detail) => (
              <div key={detail.label} className="flex items-center gap-6 group">
                <div className="w-16 h-16 rounded-2xl bg-white shadow-xl flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                  <Icon name={detail.icon} className="text-3xl" />
                </div>
                <div>
                  <div className="font-bold text-primary text-lg">{detail.label}</div>
                  <div className="text-on-surface-variant">{detail.value}</div>
                </div>
              </div>
            ))}
          </div>
        </div>
        <div className="h-[400px] md:h-[500px] rounded-[40px] overflow-hidden shadow-2xl border-8 border-white relative group">
          <div className="w-full h-full bg-surface-dim flex items-center justify-center relative">
            ...
            />
            <div className="relative z-10 text-center p-8 space-y-4 bg-white/80 backdrop-blur-md rounded-3xl border border-white mx-10">
              <Icon name="location_on" className="text-5xl text-primary animate-bounce" />
              <p className="font-bold text-xl text-primary">JIRS Global Campus</p>
              <p className="text-sm text-on-surface-variant">Kanakapura Road, Bangalore</p>
              <button className="text-primary font-bold text-sm uppercase tracking-widest border-b-2 border-primary pt-2">
                Open Maps
              </button>
            </div>
          </div>
